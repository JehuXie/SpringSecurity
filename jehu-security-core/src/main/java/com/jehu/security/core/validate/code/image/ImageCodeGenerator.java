...
public class ImageCodeGenerator implements ValidateCodeGenerator {
	...
	public ValidateCode generate(ServletWebRequest request) {
		...
		String sRand = "";
		for(int i=0;i<ServletRequestUtils.getIntParameter(request.getRequest(), "lenght", securityProperties.getCode().getImage().getLength());i++) {
			String rand = String.valueOf(random.nextInt(10));
			sRand += rand;
			g.setColor(new Color(20+random.nextInt(110),20+random.nextInt(110),20+random.nextInt(110),20+random.nextInt(110)));
			g.drawString(rand, 13*i+6, 16);
		...
		g.dispose();
		return new ImageCode(image,sRand,ServletRequestUtils.getIntParameter(request.getRequest(), "expiredIn", securityProperties.getCode().getImage().getExpiredIn()));
